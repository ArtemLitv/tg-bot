...
Запуск сервера и клиента одновременно (рекомендуется):
```
npm run dev:all
```
...
> **Важно!** Для корректной работы авторизации и админ-панели необходимо, чтобы были запущены и сервер, и клиент одновременно. Рекомендуется использовать команду `npm run dev:all`.
...
#### Ошибка "Gateway Timeout" при авторизации

Если при попытке авторизации в админ-панели вы получаете ошибку "504 Gateway Timeout", это может быть вызвано следующими причинами:

1. **Сервер не запущен**: Убедитесь, что сервер запущен и работает на порту 3001. Запустите сервер командой `npm run dev` или `npm run dev:all`.

2. **Проблемы с соединением**: Проверьте, что нет конфликтов портов и другие приложения не используют порт 3001.

3. **Долгое время ответа**: Если сервер запущен, но отвечает слишком долго, возможно, есть проблемы с базой данных или другими компонентами. Проверьте логи сервера.

#### Ошибка "ECONNREFUSED" или "AggregateError" при авторизации

Если вы видите ошибку "ECONNREFUSED" или "AggregateError" в консоли, это означает, что клиент не может подключиться к серверу. Причины могут быть следующими:

1. **Сервер не запущен**: Наиболее распространенная причина. Убедитесь, что сервер запущен и работает на порту 3001.

2. **Неправильный порт или адрес**: Проверьте, что сервер запущен именно на порту 3001 и доступен по адресу localhost.

3. **Блокировка фаерволом**: Проверьте настройки фаервола, возможно, он блокирует соединения на порту 3001.

4. **Конфликт портов**: Убедитесь, что порт 3001 не используется другим приложением.

Для решения большинства проблем рекомендуется:

1. Остановить все запущенные процессы (сервер и клиент)
2. Запустить проект заново командой `npm run dev:all` (рекомендуется)
3. Если проблема сохраняется, проверьте логи в директории `logs` или в консоли сервера
4. Убедитесь, что в файле `.env` параметр PORT установлен в значение 3001
